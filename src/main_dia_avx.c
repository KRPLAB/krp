...
int main(int argc, char *argv[]) {
	...
	inicializa_matriz_dia_avx(&a, tam, meia_banda);
	...
	for (int i = 0; i < tam; i++)
		x_true[i] = 1.0;
	double *b = matriz_banda_vetor_dia_avx(&a, x_true);
	...
	// Erro relativo...
	double *Ax_final = matriz_banda_vetor_dia_avx(&a, x);
	...
	printf("%d,%d,%u,%d,%.6e,%.6e\n", tam, bandas, seed_mat, iters,
	       t_fim - t_inicio, erro_relativo);
